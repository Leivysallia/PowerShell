...
class ScriptConfig:
    ...
    def _create_default_config(self) -> dict:
        ...
        config = {
            'categories': {
                'Files': {'icon': '📁', 'description': 'File and folder operations', 'scripts': []},
                'Computer': {'icon': '💻', 'description': 'System management and monitoring', 'scripts': []},
                'Network': {'icon': '🌐', 'description': 'Network and DNS operations', 'scripts': []},
                'Audio': {'icon': '🔊', 'description': 'Sound and speech operations', 'scripts': []},
                'Apps': {'icon': '📦', 'description': 'Application management', 'scripts': []},
                'Dev': {'icon': '⚙️', 'description': 'Development and Git tools', 'scripts': []},
                'Utils': {'icon': '🔧', 'description': 'Utility scripts', 'scripts': []},
                'Custom': {'icon': '⭐', 'description': 'Your custom scripts', 'scripts': []},
            },
            'window': {'width': 1200, 'height': 800, 'theme': 'dark'},
            'scripts_per_row': 4,
        }
    ...
    def add_script_to_category(
        self, category: str, script_name: str, display_name: str = None, description: str = None
    ):
        ...
        existing = [s for s in self.config['categories'][category]['scripts'] if s['name'] == script_name]
        if not existing:
    ...
    def remove_script_from_category(self, category: str, script_name: str):
        ...
        self.config['categories'][category]['scripts'] = [
            s for s in self.config['categories'][category]['scripts'] if s['name'] != script_name
        ]
        self.save_config()
...
class OutputConsole(ctk.CTkFrame):
    ...
    def __init__(self, parent, **kwargs):
        ...
        ctk.CTkLabel(header, text='📟 Output Console', font=ctk.CTkFont(size=14, weight='bold')).pack(side='left')

        ctk.CTkButton(header, text='Clear', width=60, height=24, command=self.clear).pack(side='right')

        # Text area with scrollbar
        self.text = ctk.CTkTextbox(
            self, fg_color='#0f0f1a', text_color='#00ff88', font=ctk.CTkFont(family='Consolas', size=12), wrap='word'
        )
...
class InputDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, prompts: List[dict], script_name: str):
        ...
        # Title
        ctk.CTkLabel(self, text=f'📝 {script_name}', font=ctk.CTkFont(size=16, weight='bold')).pack(pady=(20, 10))
        ...
        for prompt in prompts:
            ...
            ctk.CTkLabel(frame, text=prompt.get('label', prompt['name']), font=ctk.CTkFont(size=12)).pack(anchor='w')

            entry = ctk.CTkEntry(frame, placeholder_text=prompt.get('placeholder', ''), width=360)
        ...
        ctk.CTkButton(btn_frame, text='Cancel', fg_color='#4a4a6a', hover_color='#6a6a8a', command=self._cancel).pack(
            side='left', padx=(0, 10)
        )

        ctk.CTkButton(
            btn_frame, text='Run Script', fg_color='#2d8a4e', hover_color='#3da05e', command=self._submit
        ).pack(side='right')
...
class ScriptManagerDialog(ctk.CTkToplevel):
    ...
    def _create_layout(self):
        ...
        ctk.CTkLabel(left_panel, text='Available Scripts', font=ctk.CTkFont(size=14, weight='bold')).pack(pady=10)
        ...
        search_entry = ctk.CTkEntry(
            left_panel, placeholder_text='🔍 Search scripts...', textvariable=self.search_var, width=280
        )
        ...
        self.category_menu = ctk.CTkOptionMenu(
            middle, variable=self.category_var, values=list(self.config.config['categories'].keys()), width=70
        )
        self.category_menu.pack(pady=(0, 20))

        ctk.CTkButton(middle, text='Add →', width=70, command=self._add_to_category).pack(pady=5)
        ...
        ctk.CTkLabel(right_panel, text='Category Contents', font=ctk.CTkFont(size=14, weight='bold')).pack(pady=10)
        ...
        ctk.CTkButton(bottom, text='Done', width=100, command=self.destroy).pack(side='right')
    ...
    def _add_to_category(self):
        if hasattr(self, 'selected_script') and self.selected_script:
            ...
            self.config.add_script_to_category(
                category, self.selected_script, self.selected_script.replace('-', ' ').title()
            )
            self._update_category_list()
...
class PowerShellCommander(ctk.CTk):
    ...
    def _create_header(self):
        ...
        # Title
        ctk.CTkLabel(
            header, text='⚡ PowerShell Commander', font=ctk.CTkFont(size=24, weight='bold'), text_color='#ffffff'
        ...
        # Refresh button
        ctk.CTkButton(
            controls, text='🔄', width=40, fg_color='#4a4a6a', hover_color='#6a6a8a', command=self._refresh
        ).pack(side='left', padx=5)
    ...
    def _create_main_content(self):
        ...
        ctk.CTkLabel(sidebar, text='Categories', font=ctk.CTkFont(size=14, weight='bold')).pack(pady=(15, 10))
        ...
        # Quick actions
        ctk.CTkLabel(
            sidebar, text='Quick Actions', font=ctk.CTkFont(size=12, weight='bold'), text_color='#888899'
        ).pack(pady=(10, 5))
        ...
        self.scripts_frame = ctk.CTkScrollableFrame(
            content_container, fg_color='#1a1a2e', label_text='', label_font=ctk.CTkFont(size=14, weight='bold')
        )
    ...
    def _create_status_bar(self):
        ...
        self.status_label = ctk.CTkLabel(self.status_bar, text='Ready', font=ctk.CTkFont(size=11), text_color='#888899')
    ...
    def _load_category(self, category: str):
        ...
        if not scripts:
            ...
            ctk.CTkLabel(
                empty_frame, text='No scripts in this category yet', font=ctk.CTkFont(size=14), text_color='#666677'
            ).pack()

            ctk.CTkButton(empty_frame, text='📦 Add Scripts', command=self._open_script_manager).pack(pady=10)
            return
        ...
        for i, script in enumerate(scripts):
            ...
            btn = ScriptButton(row_frame, script, self._run_script, width=200, height=100)
